...
api_logger = setup_logger('dbmanager.api', level='INFO', log_file=API_LOG, console=False)
...
def log_backup_start(db_name: str, db_id: int):
    """Log backup start"""
    backup_logger.info(f"Starting backup for database: {db_name} (ID: {db_id})")


def log_backup_success(db_name: str, backup_file: str, size_mb: float, duration: float = None):
    """Log backup success"""
...
def log_backup_failure(db_name: str, error: str):
    """Log backup failure"""
    backup_logger.error(f"Backup failed for {db_name}: {error}")


def log_restore_start(db_name: str, backup_file: str):
    """Log restore start"""
    restore_logger.info(f"Starting restore for {db_name} from {backup_file}")


def log_restore_success(db_name: str, backup_file: str, duration: float = None):
    """Log restore success"""
...
def log_restore_failure(db_name: str, backup_file: str, error: str):
    """Log restore failure"""
