...
class ConfigExporter:
    ...
    def export_config(
        self, output_path: Optional[str] = None, include_backups: bool = False
    ) -> str:
        """
        ...
        if output_path is None:
            ...
            output_path_obj = Path.home() / f"dbmanager-export-{timestamp}.zip"
        else:
    ...
    def import_config(
        self, import_path: str, merge: bool = False, restore_backups: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not import_path_obj.exists():
            raise FileNotFoundError(f"Import file not found: {import_path_obj}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if merge:
                ...
                # Merge databases
                existing_db_names = {
                    db["name"] for db in current_config.get("databases", [])
                }
                ...
                existing_bucket_names = {
                    b["name"] for b in current_config.get("s3_buckets", [])
                }
                ...
                for bucket in imported_config.get("s3_buckets", []):
                    if bucket["name"] not in existing_bucket_names:
                        ...
                        bucket_copy["id"] = (
                            max([b.get("id", 0) for b in s3_buckets] + [0]) + 1
                        )
                ...
                if "global_settings" in imported_config:
                    current_config["global_settings"] = imported_config[
                        "global_settings"
                    ]
                ...
                summary["databases_imported"] = len(
                    imported_config.get("databases", [])
                )
                summary["s3_buckets_imported"] = len(
                    imported_config.get("s3_buckets", [])
                )
                summary["schedules_imported"] = len(
                    imported_config.get("schedules", [])
                )
                        ...
                        if db_dir.is_dir():
                            ...
                            for backup_file in db_dir.iterdir():
                                if backup_file.is_file():
                                    shutil.copy2(
                                        backup_file, dest_dir / backup_file.name
                                    )
    ...
    def export_to_json(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path_obj = Path.home() / f"dbmanager-config-{timestamp}.json"
        else:
            output_path_obj = Path(output_path)
