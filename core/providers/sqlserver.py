...
class SQLServerProvider(BaseProvider):
    ...
    def _can_use_docker_api(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if not is_running:
                    print(f"[DEBUG] Docker API: Container '{container_name}' not running (status: {container.status})")
                    return False
                ...
                try:
                    ...
                    if exit_code == 0:
                        print(f"[DEBUG] Docker API: Container '{container_name}' accessible ✅")
                        return True
                    else:
                        print(f"[DEBUG] Docker API: Container '{container_name}' not executable (exit: {exit_code})")
                        return False
                except Exception as e:
                    print(f"[DEBUG] Docker API: Container '{container_name}' exec failed - {e}")
                    return False
                    
            except Exception as e:
                print(f"[DEBUG] Docker API: Container '{container_name}' not found - {type(e).__name__}")
    ...
    def _backup_mssql_scripter(self, backup_dir: str, timestamp: str) -> str:
        ...
        cmd = [
            ...
            "-U", params["user"],
            "-d", params["database"],
            "-P", params["password"],  # Pass password directly
    ...
    def _backup_sql_script(self, backup_dir: str, timestamp: str) -> str:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            # Write header
            f.write(f"-- ==========================================================\n")
            f.write(f"-- SQL Server Complete Backup Script\n")
            ...
            for schema, table in tables:
                ...
                col_defs = []
                for col_name, data_type, max_len, num_prec, num_scale, nullable, default, is_identity in columns:
                    col_def = f"    [{col_name}] {data_type.upper()}"
                    
                    # Add length/precision
                    if max_len and data_type.lower() in ('varchar', 'nvarchar', 'char', 'nchar', 'varbinary', 'binary'):
                ...
                if rows:
                    ...
                    for row in rows:
                        ...
                        f.write(f"INSERT INTO [{schema}].[{table}] ({column_list}) VALUES ({', '.join(values)});\n")
            ...
            f.write(f"\n-- ==========================================================\n")
            f.write(f"-- Backup completed successfully\n")
            f.write(f"-- ==========================================================\n")
    ...
    def _restore_native_bak(self, backup_file: str) -> bool:
        ...
        if self._can_use_docker_api():
            ...
            raise RuntimeError("Cannot restore .bak file: SQL Server container not accessible via Docker API")
    ...
    def _restore_sql_script(self, backup_file: str) -> bool:
        ...
        cursor.execute("""
            ...
            WHERE TABLE_TYPE = 'BASE TABLE'
              ...
              AND TABLE_NAME NOT IN ('spt_fallback_db', 'spt_fallback_dev', 'spt_fallback_usg', 'spt_monitor', 'MSreplication_options', 'spt_values')
            ORDER BY TABLE_SCHEMA, TABLE_NAME
        """)
