...
class DBManager:
    ...
    def backup_database(
        ...
        if s3_enabled and s3_bucket_id:
            try:
                storage = self.storage_manager.get_storage(s3_bucket_id)
                if storage:
                    filename = os.path.basename(path)
                    ...
                    if storage.upload_file(
                        path, s3_key, metadata, dedup_ref_key=dedup_ref_key
                    ):
                        print(f"✅ Backup uploaded to S3: {s3_key}")

                        # Upload checksum if it exists and we didn't deduplicate.
                        # Even with dedup, the checksum file is tiny, so upload it.
                        ...
                        if checksum_file and Path(checksum_file).exists():
                            ...
                            if storage.upload_file(checksum_file, checksum_s3_key):
                                print("✅ Checksum uploaded to S3")
                    else:
                        print("⚠️  S3 upload failed, local backup retained")
            except Exception as e:
    ...
    def restore_database(
        ...
        if checksum_file.exists():
            ...
            if progress:
                progress.update(
                    message="Verifying backup integrity...", step="Pre-check"
                )
            try:
                ...
                # CLI could allow override; API/Auto should abort or warn.
