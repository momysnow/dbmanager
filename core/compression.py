...
def compress_file(file_path: str, algorithm: str = 'gzip', level: int = 6, 
    ...
    if algorithm == 'gzip':
        ...
        if not ZSTD_AVAILABLE:
            raise ValueError("zstd compression not available. Install: pip install zstandard")
...
def decompress_file(file_path: str, output_path: Optional[str] = None,
                   remove_compressed: bool = False) -> str:
    """
    ...
    if extension == '.gz':
        ...
        if not ZSTD_AVAILABLE:
            raise ValueError("zstd decompression not available. Install: pip install zstandard")
        ...
        if not LZ4_AVAILABLE:
            raise ValueError("lz4 decompression not available. Install: pip install lz4")
