...
class ConnectionParams(BaseModel):
    ...
    database: Optional[str] = Field(None, description="Database name") # Optional because some providers like S3 might not strictly use it or it might be implicit
    user: Optional[str] = Field(None, description="Database username")
    password: Optional[str] = Field(None, description="Database password")
    # Add other provider-specific fields as optional
    trust_certificate: Optional[bool] = Field(False, description="Trust server certificate (SQL Server)")
...
class DatabaseResponse(BaseModel):
    ...
    name: str
    provider: str
    params: Dict[str, Any] # Use generic dict to include all fields but processed (no password)
