...
from api.models.s3 import S3BucketCreate, S3BucketUpdate, S3BucketResponse, S3TestResult
...
async def get_s3_bucket(
    bucket_id: int, config_manager: ConfigManager = Depends(get_config_manager)
) -> S3BucketResponse:
...
@router.post(
    "/s3-buckets", response_model=S3BucketResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_s3_bucket(
    ...
    buckets = config_manager.config.get("s3_buckets", [])
    bucket_index = next(
        (i for i, b in enumerate(buckets) if b.get("id") == bucket_id), None
    )
...
async def delete_s3_bucket(
    ...
    # Remove bucket
    config_manager.config["s3_buckets"] = [
        b for b in buckets if b.get("id") != bucket_id
    ]
    config_manager.save_config()
...
async def test_s3_connection(
    bucket_id: int, db_manager: DBManager = Depends(get_db_manager)
) -> S3TestResult:
    """Test S3 bucket connection"""

    try:
        # Get bucket manager and test connection
        storage = db_manager.storage_manager.get_storage(bucket_id)
        if not storage:
            raise ValueError(f"S3 bucket with ID {bucket_id} not found")

        # Test by listing (this will verify credentials)
        storage.list_files()

        return S3TestResult(success=True, message="S3 connection successful")

    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except Exception as e:
        return S3TestResult(success=False, message="S3 connection failed", error=str(e))
